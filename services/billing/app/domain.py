...
class UnbilledReason(str, Enum):
    ...
    # Склад не ответил — отдельно от «нет количества».
    #
    # Воркер хранения ходит в `wms` за коробко-местами и любой отказ считал
    # отсутствием количества. С этапа 1, когда маршруты склада закрыли, он
    # получал 401 на каждый кабинет и каждый досчитываемый день: 162 строки
    # «нет количества» за один такт, хранение не начислено НИ ОДНОМУ клиенту.
    # Дежурный по такой причине идёт искать пустые склады, а искать надо
    # токен. Причина обязана называть то, что случилось.
