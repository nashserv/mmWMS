"""Fail-closed конфигурация роле́вой части сервиса identity.
...
def upstream_url() -> str | None:
    """Платформенный identity, который владеет JWT и JWKS.

    Поток C механику токенов не трогает (файл 04). Когда исходники боевого
    сервиса приедут на стенд, сюда приходит его адрес, и проверка токена
    делегируется ему целиком.
    """
    value = os.getenv("IDENTITY_UPSTREAM_URL", "").strip()
