...
class StockOperations:
    ...
    def available_rows(self, seller: str) -> tuple[list[dict[str, Any]], int]:
        ...
        движением `good → reserved`, поэтому в проекции `stock_balance`
        состояние `good` зарезервированного уже не содержит. Прежняя формула
        мастера вычитала его второй раз: при 10 единицах и резерве на 3 в
        Wildberries уезжало `(10−3) − 3 − 0 = 4`, а физически свободно 7.

        Занижать всегда (инвариант 7) — это про `buffer`, страховой запас
        владельца. Двойное вычитание резерва страховкой не было: оно ошибалось
        в безопасную сторону, но ценой недопродажи у клиента, и держалось
        ровно на том, что формулу переписали из 6.4 буквально. Расхождение
        нашли независимо поток 0 и поток A; решение принято, мастер исправлен
        (версия 1.3).
            ...
            with transaction(connection) as cursor:
                ...
                for row in cursor.fetchall():
                    ...
                    if not barcode:
                        dropped += 1
                        continue
                    available = int(row["good"]) - int(row["buffer"] or 0)
