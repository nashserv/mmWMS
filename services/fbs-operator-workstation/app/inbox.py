...
class InboxConsumer:
    ...
    def _consume(self) -> None:
        ...
        try:
            ...
            # Очередь объявляется БЕЗ аргументов, а границы ей ставит политика
            # брокера (`infrastructure/stand/scripts/rabbitmq-init.sh`).
            #
            # Аргументы очереди неизменяемы: RabbitMQ отвечает на повторное
            # объявление с другими аргументами отказом 406
            # `PRECONDITION_FAILED`, и канал закрывается. Рабочее место при
            # этом продолжает работать опросом — то есть выглядит исправным, —
            # а события шины не разбирает вовсе. Именно так это и случилось:
            # добавили `x-message-ttl` в объявление, очередь уже существовала
            # без него, и консьюмер молча отключился от шины.
            #
            # Политика действует на очередь снаружи и меняется без
            # пересоздания. Границы нужны: событие для рабочего места — повод
            # сходить за данными, живёт оно секунды, а без границ очередь
            # копится, пока рабочее место лежит.
