"""Обвязка тестов сервиса internal-admin."""
...
def database() -> Iterator[Database]:
    base = os.getenv("ADMIN_TEST_DATABASE_URL")
    if not base:
        raise RuntimeError("нужен ADMIN_TEST_DATABASE_URL")
    base = _guard_test_database(base, "ADMIN_TEST_DATABASE_URL")
    name = f"internal_admin_test_{uuid.uuid4().hex[:8]}"
