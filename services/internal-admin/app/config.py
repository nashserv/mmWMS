"""Fail-closed конфигурация админки.
...
def billing_url() -> str:
    """Админка не ходит в базу биллинга — только в его API.

    База на сервис (раздел 2.2): чтение чужой базы напрямую делает любую правку
    схемы биллинга поломкой админки, а правило «наценка по цепочке закреплений»
