...
# Границы очередей-уведомителей.
#
# Событие для рабочего места — повод сходить за данными, а не сами данные:
# живёт оно секунды, опрос всё равно идёт раз в секунду. Без границ очередь
# копится, пока рабочее место лежит, и после подъёма оно разбирает вчерашние
# поводы, а брокер к тому времени упирается в диск.
#
# Политика, а не аргументы объявления: аргументы очереди НЕИЗМЕНЯЕМЫ, и
# попытка объявить существующую очередь с другими закрывает канал отказом
# 406. Потребитель при этом продолжает работать опросом — выглядит исправным
# и не разбирает шину вовсе. Так и случилось с `workstation.inbox`.
declare_it PUT "policies/${VHOST}/notify-queues" \
    '{"pattern":"\\.inbox$","apply-to":"queues","priority":5,
      "definition":{"message-ttl":60000,"max-length":1000,"overflow":"drop-head"}}' \
    "policy notify-queues на *.inbox"
